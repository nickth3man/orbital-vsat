...
from src.utils.error_handler import ExportError, FileError
...
class ExportErrorTracker(QObject):
    ...
    def mark_success(self):
        ...
        if self.current_attempt:
            ...
            # Log completion
            logger.info(
                f"Export operation completed successfully: "
    ...
    def get_failure_stats(self) -> Dict[ExportOperation, Dict[str, Any]]:
        ...
        for op in ExportOperation:
            ...
            stats[op] = {
                ...
                "avg_duration": (
                    sum(a.duration() for a in op_attempts) / len(op_attempts)
                    if op_attempts else 0
                )
            }
